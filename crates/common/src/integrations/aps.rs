...
impl ApsAuctionProvider {
    ...
    fn parse_aps_slot(&self, slot: ApsSlotResponse) -> Result<Bid, ()> {
        ...
        // APS doesn't return creative HTML - generate a placeholder
        // In production, the publisher renders the creative client-side using the targeting keys
        let creative = format!(
            r#"<!-- APS Slot: {} | Size: {} | Price: ${:.2} -->
<div data-aps-slot="{}" data-aps-size="{}" data-aps-price="{:.2}">
    APS Creative for slot {} ({}x{})
</div>"#,
            slot.slot_id,
            slot.size,
            price,
...
impl IntegrationConfig for MockApsConfig {
    fn is_enabled(&self) -> bool {
        self.enabled
    }
}
...
pub fn register_providers(settings: &Settings) -> Vec<Arc<dyn AuctionProvider>> {
    ...
    match settings.integration_config::<MockApsConfig>("aps_mock") {
        Ok(Some(config)) => {
            log::info!(
                "Registering mock APS provider (bid_price: {})",
                config.bid_price
            );
            providers.push(Arc::new(MockApsProvider::new(config)));
        }
        Ok(None) => {
            log::debug!("Mock APS integration config found but is disabled");
        }
        Err(e) => {
            log::error!("Failed to load mock APS configuration: {:?}", e);
        }
    }
