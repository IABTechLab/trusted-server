...
fn generated_config_store_blocks(envelope_json: &str) -> String {
    // The blob lives in the `trusted_server_config` store under the
    // `trusted_server_config` key — the same store the entry point opens at boot
    // and the same key `ts config push` writes by default (it uses the logical
    // store id unless `--key` is given). Store id, blob key, and the registry
    // binding's `default_key` are all one value on purpose; splitting them is
    // what made local Fastly boot read an empty store.
    format!(
...
mod tests {
    ...
    /// Push-to-boot contract at the generator level: the blob must land in the
    /// store *and* under the key the Fastly entry point reads at boot.
    ///
    /// This previously asserted `trusted_server_config` was **empty** — the
    /// generator wrote the blob to a separate `app_config` store while boot
    /// opened `trusted_server_config`, so local Fastly booted against an empty
    /// store. The store id, the blob key, and `CONFIG_BLOB_KEY` are now one
    /// value, and this test pins that.
    #[test]
    fn generated_config_is_valid_toml() {
        ...
        let contents = parsed["local_server"]["config_stores"]
            [trusted_server_core::config_payload::CONFIG_BLOB_KEY]["contents"]
            .as_table()
            .expect("boot config store should be present in the generated Viceroy config");
        let blob = contents
            .get(trusted_server_core::config_payload::CONFIG_BLOB_KEY)
            .and_then(toml::Value::as_str)
            .expect("blob must be stored under the key the boot read looks up");
        assert_eq!(
            blob,
            envelope.as_str(),
        ...
        let settings = futures::executor::block_on(settings_from_config_blob(
            blob,
            &integration_secret_store(),
            &StoreName::from("trusted_server_secrets"),
        ))
        .expect("the generated blob must verify and deserialize the way boot loads it");
        assert!(
            !settings.publisher.domain.is_empty(),
            "generated blob should round-trip into usable settings"
        );
    }
