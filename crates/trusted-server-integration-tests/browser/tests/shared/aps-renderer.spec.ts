...
const APS_V1_TEST = process.env.TS_TEST_APS_V1 === "1";
...
test.describe("APS opaque renderer", () => {
    test.skip(
        APS_V1_TEST,
        "legacy renderer is absent from the v1 cutover artifact",
    );
...
test.describe("APS renderer v1 protocol", () => {
    test.skip(!APS_V1_TEST, "requires the explicit APS v1 test artifact");
...
window.startApsV1 = function(options) {
  ...
  slot.appendChild(frame);
        ...
        await expect
            .poll(async () =>
                (await messages("duplicate-success")).map(
        ...
        await expect(page.locator("#duplicate-success .existing")).toHaveCount(
            0,
        );
        expect(
        ...
        await expect
            .poll(async () =>
                (await messages("silent-case")).map(
                    (message) => message.message,
                ),
            )
