...
window.ucTag.renderAd(document, { adId: ${JSON.stringify(apsRenderer.bidId)}, pubUrl: ${JSON.stringify(publisherOrigin)} });
</script>`,
            }),
        );
        ...
        await page.evaluate(
            ({ apsRenderer: renderer, outerUrl }) => {
                ...
                // Loading only the GPT bridge keeps `pbjs` absent, forcing PUC's
                // dynamic-renderer branch and its hidden sibling frame topology.
                frame.src = outerUrl;
                slot.appendChild(frame);
            ...
            { apsRenderer, outerUrl: outerCreativeUrl },
        );
