...
/// typed errors.
///
/// Only the Spin WASM boot path constructs this (request-time reads resolve
/// through the registry composite), so it is compiled out elsewhere.
#[cfg(any(test, all(feature = "spin", target_arch = "wasm32")))]
...
impl PlatformSecretStore for SpinSecretStoreAdapter {
    async fn get_bytes(
        ...
        match spin_sdk::variables::get(&variable_name).await {
...
mod tests {
    ...
    fn config_store_handle_adapter_reads_verbatim_kv_key() {
        ...
        let value = futures::executor::block_on(adapter.get(
