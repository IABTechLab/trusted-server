...
describe('installTsAdInit', () => {
  ...
  it('keeps targeting, display, and refresh running when opportunity diagnostics throws', async () => {
    ...
    // Defining a TS slot registers a handoff, which replaces pubads.refresh
    // with the handoff wrapper. The wrapper delegates to the original, so hold
    // the spy before install — the same way `display` is captured below.
