...
import { createGptIntegrationRegistration } from './module';
...
async function fetchPageBids(
  ...
  const res = await fetch(`${PAGE_BIDS_PATH}?path=${encodeURIComponent(path)}`, {
    credentials: 'include',
  ...
  try {
    return (await res.json()) as PageBidsResponse;
  } catch (err) {
...
export function installSpaAuctionHook(): void {
  ...
  async function onNavigate(path: string): Promise<void> {
    ...
    try {
      const data = await fetchPageBids(path, controller.signal);
      if (!data) {
...
if (typeof window !== 'undefined') {
  ...
  if (typeof register === 'function') {
    Reflect.apply(register, window.tsjs, [createGptIntegrationRegistration(EMBEDDED_RELEASE_ID)]);
  }
}
