...
let osanoListenerOwner: OsanoCm | undefined;
let focusHandler: (() => void) | undefined;
let visibilityHandler: (() => void) | undefined;
const pendingSignalCancels = new Set<() => void>();
...
function readUspSignal(win: OsanoWindow): Promise<SignalResult> {
  ...
  return new Promise((resolve) => {
    ...
    const done = finishOnce((result: SignalResult) => {
      ...
      pendingSignalCancels.delete(cancelPending);
      resolve(result);
    });
    cancelPending = () => done(pendingResult());
    pendingSignalCancels.add(cancelPending);
    timer = window.setTimeout(cancelPending, API_TIMEOUT_MS);
...
/** Dispose every timer/listener owned by the active Osano consent mirror. */
export function disposeOsanoConsentMirror(): void {
  const cm = osanoListenerOwner;
  if (
    ...
    for (const [eventName, handler] of osanoEventHandlers) {
      try {
        ...
        // One vendor listener failure cannot retain the remaining owners.
      }
    }
  }
  ...
  mirrorGeneration += 1;
  for (const cancel of [...pendingSignalCancels]) cancel();
