...
impl Ec {
    ...
    /// old `host-signals` spelling, when a provider name or implementation is
    ...
    /// when the selected provider resolves to an implementation that needs
    ...
    pub fn validate_provider_selection(&self) -> Result<(), Report<TrustedServerError>> {
        ...
        // read as the block of a provider the adapter injects, so without the
        ...
        let selector_is_retired = matches!(
            ...
            Some(EcProviderSelection::Named(name)) if name == RETIRED_HOST_SIGNALS_PROVIDER_KEY
        );
        ...
        // which of its own implementations need them. Both providers built
        // into core take a passphrase, so both need one. A provider an adapter
