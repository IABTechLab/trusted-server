...
// IMPORTANT: This is the compiled/minified equivalent of installTsAdInit() in
// crates/js/lib/src/integrations/gpt/index.ts. Any logic change MUST be made
// in both places; rebuild the TypeScript bundle before updating this constant.
...
mod tests {
    ...
    fn head_injector_emits_ts_ad_init_bootstrap_without_inline_bids() {
        ...
        assert!(
            ...
            "should disable GPT initial load so display does not request ads before bid targeting"
        );
        assert!(
