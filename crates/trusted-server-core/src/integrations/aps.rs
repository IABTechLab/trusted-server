...
use crate::auction::types::{
    AdSlot, AuctionContext, AuctionRequest, AuctionResponse, Bid, MediaType,
};
...
impl ApsAuctionProvider {
    ...
    fn parse_aps_slot(
        &self,
        slot: &ApsSlotResponse,
        slot_id_map: &HashMap<String, String>,
    ) -> Result<Bid, ()> {
    ...
    fn parse_aps_response(&self, json: &Json, response_time_ms: u64) -> AuctionResponse {
        let slot_id_map = HashMap::new();
        self.parse_aps_response_with_slot_id_map(json, response_time_ms, &slot_id_map)
    }

    fn parse_aps_response_for_request(
        &self,
        json: &Json,
        response_time_ms: u64,
        request: &AuctionRequest,
    ) -> Result<AuctionResponse, Report<TrustedServerError>> {
        let slot_id_map = aps_slot_id_to_logical_slot_ids(request)?;
        Ok(self.parse_aps_response_with_slot_id_map(json, response_time_ms, &slot_id_map))
    }

    fn parse_aps_response_with_slot_id_map(
        &self,
        json: &Json,
        response_time_ms: u64,
        slot_id_map: &HashMap<String, String>,
        ...
        if let Ok(aps_response) = serde_json::from_value::<ApsBidResponse>(json.clone()) {
            ...
            for slot in aps_response.contextual.slots {
                match self.parse_aps_slot(&slot, slot_id_map) {
                    Ok(bid) => {
    ...
    fn parse_fastly_response(
        ...
        let auction_response = request.map_or_else(
            || Ok(self.parse_aps_response(&response_json, response_time_ms)),
            |auction_request| {
                self.parse_aps_response_for_request(
                    &response_json,
                    response_time_ms,
                    auction_request,
                )
...
fn aps_slot_id(slot: &AdSlot) -> String {
    slot.bidders
        .get("aps")
        .and_then(|params| params.get("slotID"))
        .and_then(Json::as_str)
        .map(String::from)
        .unwrap_or_else(|| slot.id.clone())
}

fn aps_slot_id_to_logical_slot_ids(
    request: &AuctionRequest,
) -> Result<HashMap<String, String>, Report<TrustedServerError>> {
    let mut slot_ids = HashMap::new();

    for slot in &request.slots {
        let aps_slot_id = aps_slot_id(slot);
        if let Some(existing_slot_id) = slot_ids.insert(aps_slot_id.clone(), slot.id.clone()) {
            return Err(Report::new(TrustedServerError::Auction {
                message: format!(
                    "duplicate APS slotID '{aps_slot_id}' for logical slots \
                     '{existing_slot_id}' and '{}'",
                    slot.id
                ),
            }));
        }
...
impl AuctionProvider for ApsAuctionProvider {
    ...
    fn parse_response(
        ...
        self.parse_fastly_response(response, response_time_ms, None)
    }
    ...
    fn parse_response_for_request(
        ...
        self.parse_fastly_response(response, response_time_ms, Some(request))
    }
...
mod tests {
    ...
    #[test]
    fn aps_request_uses_slot_level_aps_slot_id_when_present() {
        let config = ApsConfig {
            enabled: true,
            pub_id: "5128".to_string(),
            endpoint: "https://aax.amazon-adsystem.com/e/dtb/bid".to_string(),
        ...
        auction_request.slots[0].bidders.insert(
            "aps".to_string(),
            json!({
        ...
        let aps_request = provider.to_aps_request(&auction_request);
        let slot = &aps_request.slots[0];

        assert_eq!(slot.slot_id, "aps-slot-atf-sidebar");
        assert_eq!(slot.slot_name.as_deref(), Some("header-banner"));
    }

    #[test]
    fn aps_response_maps_external_slot_id_back_to_logical_slot_id() {
        let config = ApsConfig {
            enabled: true,
            pub_id: "5128".to_string(),
            endpoint: "https://aax.amazon-adsystem.com/e/dtb/bid".to_string(),
        ...
        let auction_response = provider
            .parse_aps_response_for_request(&response_json, 100, &auction_request)
            .expect("should parse APS response");
        let bid = auction_response.bids.first().expect("should parse APS bid");

        assert_eq!(bid.slot_id, "header-banner");
    }

    #[test]
    fn aps_slot_id_map_rejects_duplicate_external_slot_ids() {
        let mut auction_request = create_test_auction_request();
        for slot in &mut auction_request.slots {
            slot.bidders.insert(
                "aps".to_string(),
                json!({
                    "slotID": "duplicate-aps-slot"
                }),
            );
        }

        let error = aps_slot_id_to_logical_slot_ids(&auction_request)
            .expect_err("should reject duplicate APS slot IDs");

        assert!(
            matches!(
                error.current_context(),
                TrustedServerError::Auction { message }
                    if message.contains("duplicate APS slotID")
            ),
            "should reject ambiguous APS slot ID mapping"
        );
    }
    ...
    fn test_aps_slot_parsing() {
        ...
        let bid = provider
            .parse_aps_slot(&aps_slot, &HashMap::new())
