...
use crate::platform::PlatformPendingRequest;

use super::config::AuctionConfig;
use super::provider::AuctionProvider;
use super::types::{
    AuctionContext, AuctionRequest, AuctionResponse, Bid, BidStatus, ProviderErrorType,
};
...
pub struct DispatchedAuction {
    ...
    launch_failures: Vec<AuctionResponse>,
...
fn provider_launch_failed_response(provider_name: &str, response_time_ms: u64) -> AuctionResponse {
    AuctionResponse::error(provider_name, response_time_ms)
        .with_error_type(ProviderErrorType::LaunchFailed)
...
fn provider_transport_failed_response(
    ...
    AuctionResponse::error(provider_name, response_time_ms)
        .with_error_type(ProviderErrorType::Transport)
...
fn provider_timeout_response(provider_name: &str, response_time_ms: u64) -> AuctionResponse {
    AuctionResponse::error(provider_name, response_time_ms)
        .with_error_type(ProviderErrorType::Timeout)
        .with_metadata("message", serde_json::json!("Provider request timed out"))
}

fn append_drained_provider_responses<N, P>(
    responses: &mut Vec<AuctionResponse>,
    backend_to_provider: &mut HashMap<String, (N, Instant, P)>,
    make_response: fn(&str, u64) -> AuctionResponse,
) where
    N: AsRef<str>,
{
    let mut failures: Vec<(String, u64)> = backend_to_provider
        .drain()
        .map(|(_, (provider_name, start_time, _))| {
            (
                provider_name.as_ref().to_string(),
                start_time.elapsed().as_millis() as u64,
            )
        })
        .collect();
    failures.sort_by(|left, right| left.0.cmp(&right.0));
    for (provider_name, response_time_ms) in failures {
        responses.push(make_response(&provider_name, response_time_ms));
    }
}
...
impl AuctionOrchestrator {
    ...
    async fn run_parallel_mediation(
        ...
        let (mediator_response, winning_bids) = if let Some(mediator_name) = &self.config.mediator {
            ...
            if remaining_ms == 0 {
                ...
                let response_time_ms = mediation_start.elapsed().as_millis() as u64;
                let winning = self.select_winning_bids(&provider_responses, &floor_prices);
                return Ok(OrchestrationResult {
                    provider_responses,
                    mediator_response: Some(provider_timeout_response(
                        mediator.provider_name(),
                        response_time_ms,
                    )),
        ...
        while !remaining.is_empty() {
            let platform_result = match context.services.http_client().select(remaining).await {
                ...
                Err(e) => {
                    ...
                    append_drained_provider_responses(
                        &mut responses,
                        ...
                        match provider.parse_response(response, response_time_ms).await {
                            ...
                            Err(e) => {
                                ...
                                responses.push(provider_error_response(
                                    provider_name,
                                    response_time_ms,
                                    ProviderErrorType::ParseResponse,
                                    &e,
                                ));
                            }
                        }
            ...
            if auction_start.elapsed() >= deadline && !remaining.is_empty() {
                ...
                append_drained_provider_responses(
                    &mut responses,
                    &mut backend_to_provider,
                    provider_timeout_response,
                );
                break;
            }
        }

        if !backend_to_provider.is_empty() {
            log::warn!(
                "{} provider request(s) were not accounted for after select loop; marking as transport failures",
                backend_to_provider.len()
            );
            append_drained_provider_responses(
                &mut responses,
                &mut backend_to_provider,
    ...
    pub async fn dispatch_auction(
        ...
        let mut launch_failures: Vec<AuctionResponse> = Vec::new();
        ...
        for provider_name in provider_names {
            ...
            match provider.request_bids(request, &provider_context).await {
                ...
                Err(e) => {
                    ...
                    launch_failures.push(provider_launch_failed_response(
                        provider.provider_name(),
                        response_time_ms,
                    ));
                }
    ...
    pub async fn collect_dispatched_auction(
        ...
        let mut responses: Vec<AuctionResponse> = launch_failures;
        let mut remaining = pending_requests;

        while !remaining.is_empty() {
            let select_result = match context
                .services
                .http_client()
                .select(remaining)
                .await
            ...
            match select_result.ready {
                ...
                Err(e) => {
                    if let Some(ref backend_name) = select_result.failed_backend_name {
                        if let Some((provider_name, start_time, _)) =
                            backend_to_provider.remove(backend_name)
                        {
                            let response_time_ms = start_time.elapsed().as_millis() as u64;
                            log::warn!(
                                "Provider '{}' request failed during collection: {:?}",
                                provider_name,
                                e
                            );
                            responses.push(provider_transport_failed_response(
                            ...
                            log::warn!(
                                "A provider request failed during collection (backend '{}' not tracked): {:?}",
                                backend_name,
                                e
                            );
                        }
        ...
        if !backend_to_provider.is_empty() {
            log::warn!(
                "{} dispatched provider request(s) were not accounted for after collection; marking as transport failures",
                backend_to_provider.len()
            );
            append_drained_provider_responses(
                &mut responses,
        ...
        let (mediator_response, winning_bids) = self
            .run_dispatched_mediator(
                &responses,
                &floor_prices,
                &request,
                auction_start,
    ...
    async fn run_dispatched_mediator(
        &self,
        responses: &[AuctionResponse],
        floor_prices: &HashMap<String, f64>,
        request: &AuctionRequest,
        ...
        timeout_ms: u32,
        context: &AuctionContext<'_>,
    ) -> (Option<AuctionResponse>, HashMap<String, Bid>) {
        let Some(mediator_name) = self.config.mediator.as_deref() else {
            return (None, self.select_winning_bids(responses, floor_prices));
        };

        let Some(mediator) = self.providers.get(mediator_name) else {
            log::warn!("Mediator '{}' not registered", mediator_name);
            return (
                Some(provider_launch_failed_response(mediator_name, 0)),
                self.select_winning_bids(responses, floor_prices),
            );
        };

        // Cap the mediator at whichever is tighter: its own configured timeout
        // or the remaining auction budget (A_deadline). SSP backends are given
        // first_byte_timeout = effective_timeout at dispatch time, so they
        // cannot run past A_deadline independently; the mediator must also be
        // capped to preserve the bounded hold invariant.
        ...
        if remaining == 0 {
            ...
            return (
                Some(provider_timeout_response(
                    mediator.provider_name(),
        ...
        // consumed by `send_async` during dispatch, so we substitute a canonical
        // placeholder URL. Any future mediator that needs real client headers
        // must snapshot them at dispatch time onto `DispatchedAuction` rather
        // than reading `context.request` here.
        ...
        match mediator.request_bids(request, &mediator_context).await {
            Ok(pending) => match context
                ...
                Ok(platform_resp) => {
                        ...
                        .await
                    {
                        ...
                        Err(e) => {
                            ...
                            let response_time_ms = mediator_start.elapsed().as_millis() as u64;
                            let mediator_response = provider_error_response(
                                mediator.provider_name(),
                                response_time_ms,
                                ProviderErrorType::ParseResponse,
                                &e,
                ...
                Err(e) => {
                    ...
                    (
                        Some(provider_transport_failed_response(
                            mediator.provider_name(),
                            response_time_ms,
                        )),
                        self.select_winning_bids(responses, floor_prices),
            ...
            Err(e) => {
                ...
                (
                    Some(provider_launch_failed_response(
                        mediator.provider_name(),
                        response_time_ms,
                    )),
                    self.select_winning_bids(responses, floor_prices),
...
mod tests {
    ...
    #[async_trait::async_trait(?Send)]
    impl AuctionProvider for SlowParsingProvider {
        fn provider_name(&self) -> &'static str {
            self.name
        }
    ...
    struct ContextParseFailingProvider {
        name: &'static str,
        backend: &'static str,
    }

    #[async_trait::async_trait(?Send)]
    impl AuctionProvider for ContextParseFailingProvider {
        fn provider_name(&self) -> &'static str {
            self.name
        }
        ...
        async fn parse_response(
            ...
            Err(Report::new(TrustedServerError::Auction {
                message: "parse failed in test provider".to_string(),
            }))
        }
    ...
    fn provider_error_response_includes_diagnostic_metadata() {
        ...
        let response = super::provider_error_response(
            "prebid",
            37,
            super::ProviderErrorType::ParseResponse,
            &error,
        );
    ...
    // Remaining timeout enforcement paths that still need focused coverage:
    ...
    #[tokio::test]
    async fn synchronous_timeout_records_uncollected_provider_response() {
        let stub = Arc::new(StubHttpClient::new());
        ...
        let config = AuctionConfig {
            enabled: true,
            providers: vec!["slow".to_string(), "late".to_string()],
            timeout_ms: 1,
            mediator: None,
            ..Default::default()
        };
        let mut orchestrator = AuctionOrchestrator::new(config);
        orchestrator.register_provider(Arc::new(SlowParsingProvider {
            name: "slow",
        ...
        let result = orchestrator
            .run_auction(&request, &context)
            .await
            .expect("should complete with best available responses");

        let late = result
            .provider_responses
            .iter()
            .find(|response| response.provider == "late")
            .expect("should record the uncollected provider");
        assert_eq!(
            late.status,
            BidStatus::Error,
            "should expose the uncollected provider as an error response"
        );
        assert_eq!(
            late.metadata
                .get("error_type")
            ...
            Some("timeout"),
            "should tag the uncollected provider as a timeout"
        );
    }

    #[tokio::test]
    async fn dispatched_collect_preserves_launch_and_parse_failures() {
        let stub = Arc::new(StubHttpClient::new());
        stub.push_response(200, b"{}".to_vec());
        stub.push_response(200, b"{}".to_vec());
        let services = build_services_with_http_client(stub);
        ...
        let config = AuctionConfig {
            enabled: true,
            providers: vec![
                "launch-failing".to_string(),
                "parse-failing".to_string(),
                "ok".to_string(),
            ],
            timeout_ms: 2000,
            mediator: None,
        ...
        let mut orchestrator = AuctionOrchestrator::new(config);
        orchestrator.register_provider(Arc::new(LaunchFailingProvider));
        orchestrator.register_provider(Arc::new(ContextParseFailingProvider {
            name: "parse-failing",
        ...
        let dispatched = orchestrator
            .dispatch_auction(&request, &context)
            .await
            .expect("should dispatch at least one provider");

        let result = orchestrator
            .collect_dispatched_auction(dispatched, &context)
            .await;

        let launch = result
            .provider_responses
            .iter()
            .find(|response| response.provider == "launch-failing")
            .expect("should retain launch failure from dispatch");
        assert_eq!(
            launch
                .metadata
                .get("error_type")
                .and_then(serde_json::Value::as_str),
            Some("launch_failed"),
            "should tag launch failures for telemetry mapping"
        );
        let parse = result
            .provider_responses
            .iter()
            .find(|response| response.provider == "parse-failing")
            .expect("should retain parse failure from collect");
        assert_eq!(
            parse
                .metadata
                .get("error_type")
                .and_then(serde_json::Value::as_str),
            Some("parse_response"),
            "should tag parse failures for telemetry mapping"
        );
    }

    #[tokio::test]
    async fn dispatched_collect_preserves_transport_failures() {
        let stub = Arc::new(StubHttpClient::new());
        stub.push_response(200, b"{}".to_vec());
        stub.push_response(200, b"{}".to_vec());
        ...
        let config = AuctionConfig {
            enabled: true,
            providers: vec!["transport-failing".to_string(), "ok".to_string()],
            timeout_ms: 2000,
            mediator: None,
            ..Default::default()
        };
        ...
        let dispatched = orchestrator
            .dispatch_auction(&request, &context)
            .await
            .expect("should dispatch providers");
        ...
        let transport = result
            .provider_responses
            .iter()
            .find(|response| response.provider == "transport-failing")
            .expect("should retain transport failure from collect");
        assert_eq!(
            transport
                .metadata
                .get("error_type")
                .and_then(serde_json::Value::as_str),
            Some("transport"),
            "should tag transport failures for telemetry mapping"
        );
    }
