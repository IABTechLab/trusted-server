...
pub fn load_consent_from_kv(store: &KvHandle, ec_id: &str) -> Option<ConsentContext> {
    let bytes = match futures::executor::block_on(store.get_bytes(ec_id)) {
        ...
        Err(e) => {
            log::debug!("Consent KV lookup error: {e}");
            return None;
        }
    };
    ...
    match serde_json::from_slice::<KvConsentEntry>(&bytes) {
        Ok(entry) => {
            log::info!(
                "Loaded consent from KV store (stored_at_ds={})",
                entry.stored_at_ds
            );
            Some(context_from_entry(&entry))
        }
        Err(e) => {
            log::warn!("Failed to deserialize consent KV entry: {e}");
            None
        }
    }
}
...
pub fn save_consent_to_kv(store: &KvHandle, ec_id: &str, ctx: &ConsentContext, max_age_days: u32) {
    ...
    let body = match serde_json::to_vec(&entry) {
        ...
        Err(e) => {
            log::warn!("Failed to serialize consent entry: {e}");
            return;
        }
    };
...
pub fn delete_consent_from_kv(store: &KvHandle, ec_id: &str) {
    match futures::executor::block_on(store.delete(ec_id)) {
        ...
        Err(e) => {
            log::warn!("Failed to delete consent KV entry: {e}");
        }
    }
}
