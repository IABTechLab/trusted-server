...
/// Backwards-compatible name used by integrations that clear every edge-cache directive.
pub use crate::cache_policy::EDGE_CACHE_HEADER_NAMES as CDN_CACHE_HEADERS;
...
mod tests {
    ...
    fn synthesized_html_is_forced_no_store_without_validators_or_edge_headers() {
        ...
        for header_name in [header::ETAG.as_str(), header::LAST_MODIFIED.as_str()]
            .into_iter()
            .chain(SURROGATE_CACHE_HEADERS.iter().copied())
        {
            assert!(
    ...
    fn preserves_private_no_store_against_operator_cache_headers_without_cookie() {
        ...
        for header_name in SURROGATE_CACHE_HEADERS {
            assert!(
                !response.headers().contains_key(*header_name),
