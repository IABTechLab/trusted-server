        ...
        .filter_map(|(slot_id, bid)| {
            bid.price.map(|cpm| {
                ...
                match processed_adm {
                    ...
                    None => {
                        // Additionally gated on a non-blank `cache_id`: PBS
                        // reports the cache `url` and `cacheId` independently,
                        // and hb_adid falls back to a non-cache identifier
                        // (`adid`, then the bid id). Emitting the coordinates
                        // without a cache UUID would point the Universal
                        // Creative at `?uuid=<non-cache-id>` — a guaranteed
                        // cache miss. The gate matches the `non_empty` chain
                        // used for hb_adid above so a blank `cacheId` cannot
