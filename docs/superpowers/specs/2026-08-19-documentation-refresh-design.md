...
**Scope:** Documentation and doc tooling, plus startup-diagnostic log emission
for the Cloudflare and Spin smoke oracles. No public API, routing, response, or
...
- No public-API, routing, response, or successful-request behavior changes.
  Code defects found by the audits are follow-ups (list below), not in-scope
  work. A behavior-preserving private test seam or private route-descriptor
  extraction is allowed only where WP4 needs the production route registration
  set to be asserted directly; it must preserve before/after route, method,
  predicate, status, and startup behavior exactly and pass every adapter
  regression suite. The only production-side exception is emitting the
  existing Cloudflare and Spin startup failure detail to each platform's
  runtime log so negative smokes can observe the real failure. All other parity
  work is tests/tooling only.
- No new documentation toolchains beyond the `tools/docs-parity` dev
  tool and the exact-release-version-tagged external link-checker action.
...
3. The scheduled link reader has `contents: read` only and emits a bounded,
   schema-validated artifact. A separate reporter checks out the authenticated
   default-branch SHA without persisted credentials, runs only the reviewed
   reconciliation script, deduplicates one owned issue, and auto-closes it
...
4. Dependency snapshot generation reads the authenticated default-branch
   ...
   writer checks out the authenticated default-branch SHA without persisted
   credentials, runs only the reviewed submission script under `contents:
write`, and submits the fixed detector/correlator identity after
   revalidating the artifact and source SHA.
...
6. Every external action names an exact release-version tag; event, permission,
   checkout, cache, artifact, archive-member, path, mode, symlink, size, script,
   and schema invariants are enforced by static negative fixtures. Newly added
   multi-command steps live in reviewed repository scripts rather than YAML.
...
- Every smoke shares one strong oracle - exact expected status, a
  ...
  and succeeds while startup is broken, while Spin's degraded router answers
  every publisher path with a valid generic 503. Status-or-response checks
  therefore false-green on both. An adapter may emit its specific startup
  error to the runtime log for this oracle, but must not change the response
  status, response body, routing, or successful-request behavior. Cloudflare
  uses its Workers console facility. Spin's upstream EdgeZero logger initializer
  is a no-op, so the adapter owns a target-filtered `log` backend that writes
  only this error target to the component stderr stream captured by Spin.
  Axum's
  smoke is the same full clean-state
...
- Every external `uses:` reference introduced or changed in WP8a names the
  exact release-version tag corresponding to the reviewed release. WP8b
  ...
  Workflow-policy fixtures parse YAML and reject branches, commit hashes,
  malformed versions, remote local-action lookalikes, expanded permissions,
  unsafe events, unreviewed writer scripts, caller-selected tools, unbounded
  artifacts, unexpected paths or modes, symlinks, archive traversal, and
  unknown schema fields.
- Workflow concurrency uses one group per pull-request number with stale
  pull-request runs canceled, plus a separate fixed non-canceling group shared
  by scheduled and manual default-branch automation. The scheduled external-link
  reader uses `contents: read` and a 30-minute timeout. It follows at most
...
- Each reader uploads exactly one payload file to a same-run GitHub artifact:
  `link-results.zip` or `dependency-snapshot.zip`. Its writer checks out only
  `github.sha` with credentials disabled, downloads only that named same-run
  artifact, and invokes one exact reviewed repository script. The script
  requires the download directory to contain exactly that regular payload,
  verifies its SHA-256 against the reader output, and delegates closed archive,
  schema, URL, and context validation to docs-parity before using canonical
  JSON. The link ZIP
  ...
  has exactly `contents: read` plus `issues: write`, deduplicates one owned
  issue, and auto-closes it after a clean run. Workflow YAML contains only
  single-command `run` entries; multi-command orchestration lives in checked
  shell scripts, and writer validation does not invoke Python.
- The dependency ZIP is at most 4 MiB and contains exactly one regular
  ...
  enums. The reader combines bounded `cargo metadata --no-deps --locked
--offline` output with exact lockfile edges. External packages declared by a
  workspace manifest are `direct`; other reachable external packages are
  `indirect`. Development-only reachability is `development`, any runtime or
  build reachability is `runtime`, and runtime wins when both exist. Workspace
  and path packages, plus unreachable lockfile entries, are not submitted. The
  writer has exactly `contents: write`, revalidates the workflow
...
3. one dependency snapshot submission from the exact authenticated `main` SHA,
   ...
   owner/SLA;
4. final Dependabot roots and workflow action versions as observed on `main`; and
