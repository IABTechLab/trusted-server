...
moves to the top node of the `rules:` tree in `permissions.yaml` (see the
permission-model spec, §3.2 and §12).
**Author:** Engineering
...
| `permissions.yaml` top node missing `group` or `jurisdiction`                  | **Startup error.** The top node is the permission baseline for a request the geo provider leaves unmatched, so there must always be one, its `group` must name a defined group, and its `jurisdiction` states the consent handling for that request.  |
