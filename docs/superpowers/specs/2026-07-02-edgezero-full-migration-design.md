...
trusted-server is a fully EdgeZero-native app **converged on the canonical `app-demo` wiring** (decision: full convergence): every adapter binary is the one-line `run_app::<App>` entry; `App` is generated by the `edgezero_core::app!("edgezero.toml")` macro (routes from `[[triggers.http]]`, `stores()` from `[stores.*]`); core is platform-neutral; config, KV, and secrets flow exclusively through EdgeZero's `StoreRegistry`; app config is a signed blob published by `ts config push`; handlers are `#[action]` functions taking `FromRequest` extractors, with shared app state reaching them via `State<Arc<AppState>>`; and no *pre-EdgeZero* shim **and no trusted-server-local registry/dispatch wiring** remains.
...
- **(Recommended) Land P0-C + P0-D early** (alongside Phase 0 A/B), so Phase 1's adapter wiring can target `run_app` from the start and skip the throwaway local builders — Phase 1 then only does the *core* store work (composite, store-id reconciliation, boot read), and the adapters get registries free from `run_app`.
...
**Deletions (after D6/D5 resolved):** `settings_data.rs` chunk resolver, and the **Axum/Cloudflare/Spin** `platform.rs` config/secret read impls (→ write-only). **The core `PlatformConfigStore`/`PlatformSecretStore` read+write traits themselves are NOT deleted in Phase 1** — the composite *implements* them and `RuntimeServices` exposes them; they are only removed when `RuntimeServices` goes away in **Phase 4**. Phase 1 just *extracts* the write-only `PlatformConfigWriter`/`PlatformSecretWriter` sub-traits (Task 3). **Fastly's read impls stay** until Phase 5 — `legacy_main` reads through `FastlyPlatformConfigStore`/`FastlyPlatformSecretStore` via `build_runtime_services` and is live until the 100%-rollout cutover. **`management_api.rs` deletion is conditional on D6.**
...
- **App state via P0-D.** `Arc<AppState>` (Settings/orchestrator/integration registry) is registered through the P0-D `Hooks` state hook and reaches `#[action]` handlers as `State<Arc<AppState>>`. *Granularity (one `Arc<AppState>` vs per-component states) is a Phase 4 plan decision.*
