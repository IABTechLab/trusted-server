...
available; handoff suppression only handles IDs present in TS's handoff registry or
one uniquely safe hydrated-ID match. All unrelated GPT calls retain native behavior,
including supported `display()` argument forms and `refresh()` options.
