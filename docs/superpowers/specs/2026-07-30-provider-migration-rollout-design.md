...
2026-08-25. The §8 sign-off rows remain the series' decision ledger; rows the
...
| 3d  | Valid plus expired consent records: conflict resolution can select the expired record                          | Expired sources drop before conflict resolution                                                                                                                                                                                         | Open. Not addressed by the series                                                                                                                                                                                                |
...
| 3g  | Graph rows persist JA4 class, H2 fingerprint hash, and buyer-facing quality metadata                           | The draft discontinued these for new rows                                                                                                                                                                                               | **Contradicted by the series, flagged.** Rows still persist device signals, including fingerprint hash prefixes, when the opt-in providers run (#1044/#1046). Tied to the open host-signal question (sign-off 22)                |
...
| 9   | Fastly bot gate requires JA4 plus platform class before KV-backed EC writes                                    | The draft deferred host fingerprinting and startup-failed `[device] provider = "fastly"`                                                                                                                                                | **Contradicted by the series, flagged.** The `fastly` device provider ships opt-in with `builtin` (UA-only) as the default (#1044). Whether the host-signal surface stays is sign-off 22, open                                   |
...
| 9   | Defer integration-owned cookie operations from the v1 response hook; require a complete read/use/withdraw lifecycle before admission.                                                                                                                                                                                                     | hook §3                                     | (none)          | open. Overtaken (#1047 removed the whole hook from the series, so no cookie surface shipped). The deferral returns with the hook's first consumer                                                                                                           |
...
| 13  | Keep batch sync fail-closed at cutover; stage partner communication and cleanup using explicit coverage thresholds, windows, and pause actions.                                                                                                                                                                                           | rollout §6 item 6                           | (none)          | open. Not implemented (no provenance cutover exists yet)                                                                                                                                                                                                    |
...
| 25  | Enforce a stored-jurisdiction/provenance horizon for batch sync: moves into stricter regimes fail closed by the horizon, and moves out require a live visit.                                                                                                                                                                              | permission §7                               | (none)          | open. Not implemented                                                                                                                                                                                                                                       |
