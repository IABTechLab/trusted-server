...
| 13  | Cookies minted by graphless deployments have no graph row                                                                                                                           | Recognized rowless cookies are **expired and re-minted** through the ordinary graph-backed path (providers spec §5) — never adopted, since prefix-only verification cannot authenticate suffix variants; identity continuity is deliberately lost; withdrawal works without any row via the prefix-derived family ID | **Declared** — pre-existing identities restart rather than carry over                                                                                                                                  |
...
| 23  | DataDome security exemption: tag injection, cookie/ClientID read, vendor egress, and cross-integration visibility operate outside the permission model as a ratified security-purpose carve-out with owned cookie names, scope, and withdrawal semantics                                                                                  | hook §4a; permission §7                     | maintainers + legal   | open                                        |
