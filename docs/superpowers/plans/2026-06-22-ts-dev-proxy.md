...
- Default `Host` upstream is **`FROM`** (preserve production host); `--rewrite-host` sends `Host = TO`. SNI is always the `TO` host, **port stripped**. To reach a server by IP, keep `TO` a hostname and pin its connection with `--resolve HOST:IP`.
...
pub struct ProxyArgs {
    ...
    /// Shorthand single-rule TO (`HOST[:PORT]`; pairs with `--from`). Keep it a
    /// hostname so the TLS SNI/certificate stay valid; pin a connection address
    /// with `--resolve` to reach a specific server by IP.
    ...
    /// always the `--to` host; to reach a server by address, pin it with
    /// `--resolve`.
    #[arg(long)]
    pub rewrite_host: bool,

    /// Pin a host's upstream connection to an address instead of DNS (repeatable;
    /// like curl's `--resolve`). Keeps `--to` a hostname while the socket dials
    /// the given IP. Format: `HOST:IP`.
...
- Produces:
  ...
  - `struct Rule { from: String, to: Authority, rewrite_host: bool, plaintext: bool }` — `rewrite_host` false (default) sends `Host: FROM`, true sends `Host: TO`; SNI is always the `TO` host.
...
mod tests {
    ...
    fn matching_is_case_insensitive_and_port_stripped() {
        let table = RuleTable(vec![rule("www.example-publisher.com", "to.edgecompute.app", false, false)]);
    ...
    fn rewrite_default_preserves_from_host_and_sets_sni_to_to() {
        let r = rule("www.example-publisher.com", "to.edgecompute.app:8443", false, false);
...
- Produces:
  - `struct ResolvedConfig { rules: RuleTable, listen: SocketAddr, allow_non_loopback: bool, launch: Vec<Browser>, insecure: bool, basic_auth: Option<BasicAuth>, ca_dir: PathBuf, resolve: HashMap<String, IpAddr> }`.
...
mod tests {
    ...
    fn rewrite_host_uses_to() {
        ...
        let cfg = resolve(&args).expect("should resolve");
        assert!(
            cfg.rules.first_match("www.example-publisher.com").expect("rule").rewrite_host,
    ...
    fn resolve_pins_host_to_ip() {
        ...
        assert_eq!(
            ...
            Some(&"192.0.2.10".parse().expect("ipv4")),
...
pub struct ResolvedConfig {
    ...
    pub ca_dir: PathBuf,
    /// `--resolve` DNS pins: lowercase hostname → connection address.
...
/// Splits on the first `:` so the (possibly IPv6) address keeps its colons.
fn build_resolve(args: &ProxyArgs) -> Result<HashMap<String, IpAddr>, ConfigError> {
    ...
    for entry in &args.resolve {
        ...
        if !is_valid_host(&host) {
            return Err(ConfigError::Resolve { value: entry.clone() });
        }
        map.insert(host, ip);
    }
    Ok(map)
...
fn make_rule(from: &str, to: &str, rewrite_host: bool, plaintext: bool) -> Result<Rule, ConfigError> {
    ...
    Ok(Rule { from, to, rewrite_host, plaintext })
}
...
async fn matched_host_is_rewritten_and_forwarded() {
    ...
    // upstream addr, rewrite_host = false (default), insecure = true.
