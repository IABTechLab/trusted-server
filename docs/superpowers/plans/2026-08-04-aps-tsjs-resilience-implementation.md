...
`58532d7c42d2a59f8e87f0d643f8d0b109a680cb` on 2026-08-21. Its rc-owned package and
...
- [ ] **Step 2: Produce an overlap inventory before committing.** For every
  ...
  Final refresh inventory for `58532d7c42d2a59f8e87f0d643f8d0b109a680cb`:
  ...
  | GPT diagnostics requested/fill/outer-size evidence           | Preserve target-window scheduling, cancellable ownership, bounded badge size lists, shared size formatting, rounded observed boxes, destroyed-state guards, and record-currentness checks. The sole GPT owner emits a release-private requested-format fact immediately before the exact request; authenticated first-display handoff data seeds the same fact before replayed callbacks; the deferred presentation owns the only slot-size observer. | GPT diagnostics fact, observer, presentation, slot-size, store, and public-type suites; release source graph         |
  | Inactive publisher cache policy and revalidation             | Preserve the rc private/short-browser cache decisions, origin validators, replacement-304 finalization, and ad-stack eligibility gates without adding a separate cache feature to TSJS or APS.                                                                                                                                                                                                                                                        | Publisher cache-policy and response-privacy suites                                                                   |
  | CLI volatile-slot collision audit                            | Preserve the rc audit evidence, generation policy, and command contracts as release-branch behavior. Add only the hard-cutover `creative_opportunities.enabled` field required by the final config parser; do not add EdgeZero feature work.                                                                                                                                                                                                          | Host CLI audit/generate/run suites                                                                                   |
  | Duplicate APS runner-helper cleanup                          | Keep one fixed-target live runner proxy helper. Do not restore the superseded publisher-native helper, vendor bytes, pin bytes, or add a success-cache contract.                                                                                                                                                                                                                                                                                      | APS Rust proxy tests, hard-cutover scan, vendor-boundary release test                                                |
