...
- Adds site metadata with publisher domain, a validated publisher-owned page URL with query and fragment removed, and `site.publisher` from the domain; raw browser `Referer` is not forwarded as `site.ref`. This privacy-first behavior applies to every auction path and can reduce contextual targeting or per-page reporting for sites whose page identity depends on query parameters
