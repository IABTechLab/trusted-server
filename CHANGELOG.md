...
- **Breaking** — Auction creative processing is now fully opt-in: the new `[auction].sanitize_creatives` and the existing `[auction].rewrite_creatives` both default to `false`, so winning-bid `adm` ships exactly as the bidder returned it (subject to the 1 MiB per-creative cap) unless a flag is enabled. Deployments that relied on unconditional sanitization or default-on rewriting must set the flags explicitly before upgrading. The creative iframe sandbox no longer grants `allow-same-origin`, restoring origin isolation; rewritten-click recovery from the resulting opaque-origin iframe uses the GET `/first-party/proxy-rebuild` navigation fallback, now registered in every adapter. Rollback: a `true` value is serialized into the config blob and older binaries reject unknown fields — before rolling back to a binary that predates a flag, set it to `false`, push the default-compatible blob, then roll back.
