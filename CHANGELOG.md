...
- **Breaking** — `POST /auction` now forwards only a validated publisher-owned page URL as `site.page`, removing query and fragment data. The raw browser `Referer` is no longer forwarded as `site.ref`, including for Prebid Server traffic.
