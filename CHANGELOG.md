...
- **Breaking** — Replaced the legacy APS contextual integration with APS OpenRTB at `/e/pb/bid`. APS configuration now requires the canonical string `account_id`; the former account key, numeric identifiers, aliases, mixed legacy shapes, APS-specific creative-opportunity slot blocks, and retired Prebid server bidder lists are rejected. APS must be represented only by an auction provider whose profile is `aps` and, for explicit routing, its auction bidder route. APS defaults script creative eligibility off. Operators must update the endpoint, disable native APS demand for Trusted Server cohorts, and prepare GAM/Universal Creative targeting for `hb_bidder=aps` before rollout. APS renderer winners now preserve the upstream bid `id`, omit `crid` when APS omits it, and carry `ext.trusted_server.renderer` instead of `adm`; external `/auction` consumers must support this response shape.
...
- **Breaking** — The SPA re-auction endpoint is now `/_ts/page-bids`, joining every other internal route in the `/_ts/` namespace. The former double-underscore path is not a functional alias: every adapter reserves it only to return a local `404 no-store`, preventing publisher-origin fallthrough, and the shipped client never retries it. Audit `[[handlers]]` for patterns broad enough to cover `/_ts` (for example `^/_ts`), which would put this browser-facing endpoint behind Basic Auth and return `401` to every visitor; scope them to `^/_ts/admin`. Deploy the canonical client and route atomically, and roll back their binary and configuration together.
